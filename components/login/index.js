  ...
  return (
    <div className='h-screen w-full overflow-hidden flex flex-col items-center justify-center'>
      ...
      <form className='mt-8 flex flex-col items-center w-1/2 sm:w-1/3' onSubmit={login}>
        ...
        <input className='text-2xl border-2 border-gray px-3 py-4 rounded-md w-full' autoComplete='off' autoCapitalize='off' data-lpignore="true" type='text'
          placeholder='Nome utente'
          onChange={({ target }) => credentials.username = target.value}
        />
        
        <input className='mt-6 text-2xl border-2 border-gray px-3 py-4 rounded-md w-full' autoComplete='off' autoCapitalize='off' data-lpignore="true" type='password'
